...
class Canvas {
  ...
  private DrawGradient(): void {
    let gradient: Style = this.context.createRadialGradient(
      this.size.width / 2,
      this.size.height / 2,
      this.size.width / 2,
      this.size.width / 2,
  ...
  private SetupGrid(gridSize: number, style: Style) {
    this.drawnElement.push(new Grid({width: gridSize, height: gridSize}, this.size, style));
  }

  private SetupStraightSquares(gridSize: number) {
    straightSquareSettings.forEach(settings => {
      this.drawnElement.push(new StraightSquare({width: gridSize, height: gridSize}, this.size, settings.anchorPoint, settings.cellPosition));
    });
  }
  ...
  private SetupTiltedSquares() : void {
    ...
    let settings: TiltedSquareSettings = {
      ...
      speed: 0.05,
      size: {width: this.size.height - 200, height: this.size.height - 200},
      strokeSize: 25,
    };
    ...
    for (let i = 0; i < 3; i++) {
      settings.size = {width: this.size.height - 300 - 100 * i, height: this.size.height - 300 - 100 * i};
